...
class SlackEventsRagTest < ActionDispatch::IntegrationTest
  ...
  test "the case of invalid blocks error" do
    slack_ts = Time.now.to_f.to_s
    channel = conversations(:magellan_rag)
    user = users(:one)
    query_body = "ZZZ"
    ...
    stub_slack_api(:post, "chat.postMessage").to_return(
      body: { "ok" => false, "error" => "invalid_blocks" }.to_json
    ).then.to_return(
      body: { "ok" => true, "ts" => Time.now.to_f.to_s }.to_json
    )
    ...
    expected_answer = <<~END_ANSWER
      花王株式会社_エッセンシャルの事例では、TVCMの残存週が他社（11週）と比べて13週と長いと報告されています。
      詳細は以下のファイルで確認できます。
      - ファイル名: 【花王様】エッセンシャル_初回レポート報告_20221125.pdf
      - ファイルのURL: <https://drive.google.com/file/d/1MB_IerrxHZ_Dn3ziT7vPixaOF_ng84D3/preview?authuser=0|こちら>
      - 該当部分: 「15秒・30秒・60秒ともに他施策と比べて効率は良好。残存週は他社（11週）と比べて、13週と長い。」

      花王株式会社_ブランド横断の事例では、TVCMの残存週が業界傾向値（11週）と比べて13週と長いと報告されています。
      詳細は以下のファイルで確認できます。
      - ファイル名: 【花王様】横断レポート_20230116.pdf
      - ファイルのURL: <https://drive.google.com/file/d/1_t9ldOf-KcHxtC2fUS3sqCEJm8HwxMmU/preview?authuser=0|こちら>
      - 該当部分: 「残存週は業界傾向値（11週）と比べて、13週と長い。」
    END_ANSWER
    expected_response = "<@#{user.slack_id}> 回答は次のとおりです。\n\n#{expected_answer}"
    expected_answer_blocks = [
      {
        "type" => "section",
        "text" => {
    ...
    actual_body = nil
    assert_slack_api_called(:post, "chat.postMessage", times: 2) do |request|
      actual_body = decode_slack_client_request_body(request.body)
    end
